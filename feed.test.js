...
describe('Search, Sort, and Pagination API', () => {
  ...
  it('should return all sorted posts if no search query provided with pagination', function (done) {
    chai.request(server)
      ...
      .end((err, response) => {
        console.log(response.body.paginatedResults.data);
        expect(response).to.have.status(200);
  ...
  it('should return posts sorted by dateLastEdited', function (done) {
    chai.request(server)
      ...
      .end((err, response) => { 
        console.log((response.body.paginatedResults.data));
        expect(response).to.have.status(200);
        expect(Date.parse(response.body.paginatedResults.data[1].dateLastEdited)).to.be.greaterThan(Date.parse(response.body.paginatedResults.data[0].dateLastEdited));
        expect(response.body.totalSearchedResults).to.be.equal(25);
        done();
      });
  });
